...
// ---- 四视图机位（同 templeqa-render 口径）----
for (const it of insts) {
  const { mn, mx } = bboxOf(it.file);
  const pf = placeFn(it.position, it.rotY);
  ...
  for (const [view, [p, t, lens]] of Object.entries(loc)) {
    const P = pf(...p), Tg = pf(...t);
    shots.push({ name: `${it.family}-${it.id}-${view}-${TAG}`, glbs: [it.file], pos: P.map((v) => +v.toFixed(3)), tgt: Tg.map((v) => +v.toFixed(3)), lens });
  }
}
...
if (WANT_CLOSEUPS) {
  const OUTZ = path.join(ROOT, process.env.SMALLQA_ZONES || 'out-zone');
  // 亭檐角饰冒瓦最重的 428196085（0.074 m），斜俯檐角
  {
    const p = Q.pavilionInstances().find((x) => x.id === 'bld-428196085');
    const pf = placeFn(p.position, p.rotY);
    ...
    for (const [nm, lp, lt] of [
      ...
      const P = pf(...lp), Tg = pf(...lt);
      shots.push({ name: nm, glbs: [f], pos: P.map((v) => +v.toFixed(3)), tgt: Tg.map((v) => +v.toFixed(3)), lens: 50 });
    }
  }
  // 椽头（反向闭合盒）428196091 檐下近景
  {
    const p = Q.pavilionInstances().find((x) => x.id === 'bld-428196091');
    const pf = placeFn(p.position, p.rotY);
    const f = path.join(ROOT, 'out-garden-kits', 'pavilion-bld-428196091', 'model.glb');
    const P = pf(2.9, 3.3, 0.1), Tg = pf(3.0, 3.25, 0.05);
    shots.push({ name: `closeup-pav-rafter-428196091-${TAG}`, glbs: [f], pos: P.map((v) => +v.toFixed(3)), tgt: Tg.map((v) => +v.toFixed(3)), lens: 50 });
  }
  // 烤架晾杆悬空（模块本地机位，放在原点）
  {
