...
## wave4-huxinting2 格扇配色（2026-09-25，主控：格扇偏暗，与厅堂套件统一配色；几何不变，只改材质）
- `sst-timber-darkred`：底色直接 = sRGB #8a4030（hall-kit 现行 timberSrgb；主控定：与背靠背的仰山堂一致，初稿 #6a2e22 作废），不再乘 `wood-stain-color.jpg`（同 hall-kit `hk-timber-darkred` 做法；旧版贴图 × 色的有效底色约 sRGB(26,5,3)，格扇整面读成黑），木纹只走 Wood092 法线图。
