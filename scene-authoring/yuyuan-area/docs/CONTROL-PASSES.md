...
- `scripts/check-control-passes.py`：深度单调性只计同一物体内倒置（≤ 2）；终帧目标像素门槛按镜头（④⑧ 25%、⑦ 15%、⑩ 10%、⑤ 8%，其余 5%），顶部天空余量 3%
...
- 深度单调性抽查改判（主控定，选项 2，全局）：只计同一分割物体内的倒置，上限 `DEPTH_SAME_OBJ_INV_MAX = 2`
  （12 组渲染实测最大 2 = ⑧大殿重檐）。旧口径在⑦门洞看穿（戏台 20 m / 大殿 37 m / 门楣 17 m）与⑨塔楼多层檐
  （25 m 檐层与 33 m 墙交错）上误报 1 + 14 帧，全部是跨物体遮挡边界；新口径 12 组 0 报错、无镜头特例。
  负对照 `tests/control-depth-check-test.py`（进 npm test）：合成取样带翻转报错；真实街景帧①f000 / ⑪f005 翻转后同物体倒置 3 / 6 次报错。
