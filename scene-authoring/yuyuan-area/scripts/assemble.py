...
if os.environ.get('HALL_KIT') == '1':
    ...
    lay_obj = {o['id']: o for o in LAYOUT['objects']}
    HALL_KIT_IDS = json.load(open(os.path.join(ROOT, 'modules', 'hall-kit', 'ids.json'), encoding='utf-8'))['ids']
    for hall_id in HALL_KIT_IDS:
        o = lay_obj[hall_id]
        fr = hk_frame.hall_frame(o, HK_DEFAULTS)
        cx, cz = fr['centroid']
        rot_y = fr['rotY']
        ...
        if os.path.exists(coll_path):
            ...
            for b in cc.get('colliders', []):
                ...
                world_boxes.append({'name': b['name'], 'center': [round(wx, 3), round(ly, 3), round(wz, 3)],
                                    'size': b['size'], 'type': b.get('type', 'box'), 'rotY': round(rot_y, 6)})
            hall_world_records.append({'source': 'out-garden-kits/hallkit-%s/collision.json (instance space)' % hall_id,
                                       'instance': {'id': hall_id, 'position': [round(cx, 3), round(cz, 3)], 'rotY': round(rot_y, 6)},
                                       'colliders': world_boxes})
            print('hall-kit collision world boxes:', hall_id, len(world_boxes))
    # 每栋一条实例记录（instances[]），export-collision 逐栋互核
    json.dump({'instances': hall_world_records}, open(os.path.join(OUT, 'hallkit-collision-world.json'), 'w'),
