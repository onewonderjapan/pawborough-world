"""Fangbang↔temple BRIDGE review scene (BLENDER evidence path, separate from
the WebGL captures). Imports the assembled world exactly as the dataset
places it:

  street assembly GLB (world/street-reviewed.glb, identity)
  + west-extension surface + seal wall (authored in place)
  + east tail surface (authored in place)
  + the 8 temple-axis GLBs at T + R(yaw) (DESIGN_SPEC)
  + the active west/east placeholder districts as neutral gray extrusions
    (same rule as blockViews.makePlaceholder: replaced placeholders skipped)

Eight lead cameras (cameras.json, verticalFovDegrees with the same
measured-fov guard as the sibling scenes), PBR for all views + one clay pair
(shanmen-from-road). Cycles CPU, AgX, 1600x900 (budgets.blenderDevice).

Run:
  blender -b --factory-startup -t 4 -P kit/build_fangbang_scene.py -- \
      --out kit/out/fangbang-temple/renders [--samples 24]
"""
import argparse
import json
import math
...
TASK = ROOT.parent
DS_DIR = ROOT / 'world' / 'fangbang-temple'
...
# temple axis at T + R(yaw)
temple_loc = (T[0], -T[2], T[1])
for a in DS['templePlacement']['assets']:
    off = a['localOffset']
    # per-asset offset in GLB -> Blender: (ox, -oz, oy) rotated by -yaw about Z
    c, s = math.cos(YAW), math.sin(YAW)
    ox, oy, oz = off[0], off[1], off[2]
    wx = c * ox + s * oz
    wz = -s * ox + c * oz
    import_glb(DS_DIR / 'temple-axis' / a['glb'],
               loc_bl=(temple_loc[0] + wx, temple_loc[1] - wz, temple_loc[2] + oy),
               rot_z=-YAW)
...
sidecar = {'source': 'kit/build_fangbang_scene.py', 'engine': 'cycles-cpu',
           ...
           'assembly': ('street assembly identity + west-extension surface/seal-wall in place '
                        '+ east tail surface in place + 8 temple-axis GLBs at T+R(yaw) '
                        f'+ {n_boxes} active placeholder gray boxes'),
           'views': [], 'note': 'BLENDER evidence; WebGL captures are separate (kit/out/fangbang-temple/web)'}

CLAY_VIEWS = {'shanmen-from-road'}
PLAN = [(c['id'], tag) for c in cams['cameras']
        for tag in (['pbr', 'clay'] if c['id'] in CLAY_VIEWS else ['pbr'])]
...
for vid, tag in PLAN:
    ...
    print(f'RENDERED fangbang-{vid}-{tag} fov={got:.3f}deg')
...
print(f'FANGBANG_SCENE_READY views={len(PLAN)} out={args.out}')
