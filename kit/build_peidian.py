"""Peidian (side hall, 配殿) builder — temple-axis expansion batch N2.

Local frame: front-wall center bottom origin, facade +Z, depth -Z. The world
assembly instances it twice (west/east, yaw ±π/2, DESIGN_SPEC.peidian.placement).
...
Groups exported: 'peidian-body' + 'temple-ground' (base slab + step tops ARE
the physics ground — the ground naming contract).

Run:
  blender -b --factory-startup -t 4 -P kit/build_peidian.py -- \
      --config kit/peidian.config.json --out kit/out/peidian
"""
import argparse
import hashlib
import json
...
def validate(c):
    ...
    # frozen numbers (DESIGN_SPEC.peidian) — copy exact
    FROZEN = {'facadeWidthM': 6.6, 'bays': 3, 'bayM': 2.2, 'bodyDepthM': 4.6,
              'sideWallX': 3.45, 'rearWallZ': -4.6}
    for k, v in FROZEN.items():
        if bd[k] != v:
            pr.append(f'body.{k} must stay the frozen value {v}')
    if sorted(fr['columnsX']) != [-3.3, -1.1, 1.1, 3.3]:
        pr.append('front.columnsX must be the frozen ±1.1/±3.3')
    if fr['galleryColumnsLocalZ'] != 0.9 or fr['eaveOverhangToLocalZ'] != 1.5:
        pr.append('front column z 0.9 / eave overhang 1.5 are frozen')
    if (rf['ridgeY'], rf['eaveY'], rf['ridgeLocalZ'], rf['widthM']) != (6.2, 3.9, -1.85, 6.6):
        pr.append('roof ridge 6.2 / eave 3.9 / ridgeZ -1.85 / width 6.6 are frozen')
    if rf['frontEaveZ'] != fr['eaveOverhangToLocalZ'] or abs(rf['rearEaveZ'] - (bd['rearWallZ'] - 0.5)) > 1e-9:
        pr.append('roof eaves must match overhangs (front to z=1.5, rear 0.5 past the rear wall)')
    ...
    if '硬山' not in bd['gableType'] or '观音兜' not in bd['gableType']:
        pr.append('gableType must record the 硬山 ruling (观音兜 not copied)')
    ...
    if c['plaque']['widthM'] != 2.2 or c['plaque']['heightM'] != 0.7 or c['plaque']['centerY'] != 3.15:
        pr.append('plaque 2.2x0.7 @3.15 is frozen')
    return pr
...
for k in range(st['count']):
    top = (k + 1) * st['riserM']
    L.box('peidian-step', (0, top - st['riserM'] / 2, bz0 - (k + 0.5) * st['treadM']),
...
# rear wall (group peidian-body)

L.GROUP = 'peidian-body'
cs = fr['columnSizeM']
wall_top = rf['eaveY'] - 0.05

# four gallery columns with plinths (freestanding, in front of the wall)
for x in fr['columnsX']:
    L.box('peidian-column', (x, cs[1] / 2 + bs['topY'], fr['galleryColumnsLocalZ']),
          (cs[0], cs[1], cs[2]), 'wood', .01, True)
    L.box('peidian-column-plinth', (x, bs['topY'] + .17, fr['galleryColumnsLocalZ']),
          (cs[0] + .16, .34, cs[2] + .16), 'stone', .012, True)
by0, by1 = fr['topBeamY']
L.box('peidian-top-beam', (0, (by0 + by1) / 2 + bs['topY'], fr['galleryColumnsLocalZ']),
      (2 * bd['sideWallX'] - .1, by1 - by0, .26), 'wood', .01)
L.box('peidian-wall-plate', (0, rf['eaveY'] - .18, 0.02), (2 * bd['sideWallX'] - .1, .3, .24),
      'wood', .01, True)
...
for sgn in (-1, 1):
    L.box('door-leaf', (sgn * dw_w / 4, cb['door']['leafSizeM'][1] / 2 + thr[1], lz),
          ...
          'dark', .008, True)
for sgn in (-1, 1):
    L.box('door-frame-trim', (sgn * (dw_w / 2 + .045), thr[1] + 1.3, -0.05),
          (.09, 2.6, .13), 'wood', .008)
...
for sdir, z_end in ((1, ZF), (-1, ZR)):
    ...
    L.mesh('peidian-roof-shell', grid_t + grid_b, faces, 'roof', uvs + uvs)
...
while x <= HW - rf['tileRibSpacingM'] / 2 + 1e-9:
    for slope_dir, z_end in ((1, ZF), (-1, ZR)):
        _rib_objs.append(C.rib_tube(L, 'peidian-tile-rib', roof_y, x, .1, 1.0,
...
L.box('peidian-ridge-beam', (0, rf['ridgeY'] + rbh / 2, Z_MID), (rf['widthM'] + .1, rbh, rbw), 'roof', .012)
...
for rec in L.COLL:
    cx, cy, cz = rec['center']
    sx, sy, sz = rec['size']
    adapter_coll.append({'name': rec['name'], 'group': rec.get('group', 'peidian-body'),
                         'type': 'box',
...
INTERIOR = {'x': 2.9, 'y0': 0.55, 'y1': 4.0, 'z0': -4.3, 'z1': -0.3}
EPS = 1e-6
clashes = []
...
TARGETS = [(GLB_NAME, ('peidian-body', 'temple-ground'))]
out = a.out
...
if pT['triangles'] > BUD['peidianTrisMax']:
    print('BUDGET_FAIL peidianTris', pT['triangles'])
    sys.exit(5)
...
(out / 'materials.json').write_text(json.dumps(L.META, ensure_ascii=False, indent=2) + '\n',
                                    encoding='utf-8')
measure['budgets'] = {
    'peidianTris': {'actual': pT['triangles'], 'limit': BUD['peidianTrisMax'], 'pass': True},
...
measure['design'] = {
    ...
    'gable': '硬山 strips to the roof soffit; 观音兜 NOT copied (owner ruling)',
...
grid = {'source': 'kit/build_peidian.py roof_y (two-slope 硬山 shell, monotone by construction)',
