...
packageF.cameraFix). Saves kit/out/scene-v2/scene-v2.blend.
...
p.add_argument('--out', type=Path, default=Path('kit/out/scene-v2'))
...
for c in cams_of('world/fangbang-temple-v2/cameras.json'):
...
for a in _v3m['templeAxis']['assets']:
    ...
    n_axis += 1
# trees: temple-local positions -> world via the bridge transform
...
blend_path = args.out / 'scene-v2.blend'
...
(args.out / 'scene-v2-report.json').write_text(json.dumps(sidecar, indent=2) + '\n',
                                               encoding='utf-8')
print(f'SCENE_V2_READY cameras={len(CAMERAS)} renders={len(sidecar["renders"])} '
