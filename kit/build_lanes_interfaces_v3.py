"""Lanes interfaces builder v3 (lane-b-polish batch) — mouth A IDENTICAL to
the delivered v6 builder (kit/build_lanes_interfaces_v2.py, A-section code
copied verbatim); mouth B rebuilt from MEASURED wall lines with the
PRODUCTION transform semantics. Output goes to the NEW dataset dir
world/lane-b-polish/interfaces (the v6 asset stays untouched).

Mouth B defects being fixed (proven by tools/lane_b_polish_measure.py --phase
before, 2026-09-20):
  1. b-apron / b-drain-outlet were fed to quad() as [x, y, z] triples where it
     expects [x, z] pairs — the geometry collapsed onto z = 0.09 near the
     world origin (lane-local ls about -12.5), leaving the real mouth band
     with NO walkable interface ground.
  2. B_RIGHT was (cos yaw, +sin yaw) but production obbToWorld/yawMatrix maps
     local +X to world (cos yaw, -sin yaw); the pier-base bricks landed at the
     MIRRORED positions (east brick inside the walkway at lx about +0.36).
  3. The apron was authored +-1.5 wide, ignoring the real S05/S07 mouth.

Measured mouth anchors (assembly survey 2026-09-20, y=1.5 slices + collider
OBBs, lane-local frame of holder T=[57.418,0.09,14.2485] yaw -0.4818):
  S05 left-side wall lane face  lx = -1.096 (parallel to the lane axis,
                                south end ls = -0.16, wall body lx -1.096..-0.816)
  S07 right-side lane face line lx(ls) = 1.067 - 0.0536*(ls + 0.26)
                                (south end about ls -0.08, face to lx 1.34)
  street paving-frontage reaches ls -0.06 only near the west corner
  (lx about -1.2); elsewhere its south edge is ragged up to ls -1.3.

New mouth B (all world-space, production transform):
  b-apron        paving quad y=0.088 (2 mm under the street paving so the
                 ragged seam never z-fights; steps <= 2 mm), spanning the real
                 mouth: west tucked 1 cm into S05, east tucked 1 cm into S07,
                 north to ls -1.50 (under the paving), south to ls +0.005
                 (under the module floor edge).
  b-drain-outlet recessed stone channel y=0.07 (18 mm below the apron) across
                 the walkable width at ls -0.55..-0.35.
  b-pier-base-w/e brick bases at the REAL corner positions (production
                 bpt(+-0.925, -0.20)), grounded y 0..0.6, closing the slit
                 between the S05/S07 wall ends and the module portal piers.

Run: blender -b --factory-startup -t 4 -P kit/build_lanes_interfaces_v3.py -- --out world/lane-b-polish/interfaces
"""
import argparse
import json
import math
...
# ================================================================ mouth B (REBUILT from measured lines)
# production transform (src/world/collisionAdapter.js obbToWorld):
#   worldX = Tx + cos(yaw)*lx + sin(yaw)*ls
#   worldZ = Tz - sin(yaw)*lx + cos(yaw)*ls
BC = [57.418, FLOOR_Y, 14.2485]
B_YAW = -0.4818
B_C, B_S = math.cos(B_YAW), math.sin(B_YAW)


def bpt(lx, ls, y=FLOOR_Y):
    return [BC[0] + B_C * lx + B_S * ls, y, BC[2] - B_S * lx + B_C * ls]


# measured mouth faces (survey 2026-09-20): S05 face parallel at lx=-1.096
# (wall body to lx -0.816, south end ls -0.16); S07 lane face line tilts
# 0.0536 per metre, lx(-0.26) = 1.067; module portal wall outer face s=-0.12.
S05_FACE = -1.096
S07_FACE_AT = lambda ls: 1.067 - 0.0536 * (ls + 0.26)
APRON_Y = FLOOR_Y - 0.002        # 2 mm under the street paving: no z-fight seam
OUTLET_Y = FLOOR_Y - 0.02        # 18 mm under the apron: recessed drain channel

# apron: real mouth band, tucked 1 cm INTO the flanking walls, north under the
# ragged street paving edge, south under the module floor edge
apron = [
    bpt(S05_FACE - 0.01, -1.50, APRON_Y),
    bpt(S07_FACE_AT(-1.50) + 0.01, -1.50, APRON_Y),
    bpt(S07_FACE_AT(0.005) + 0.01, 0.005, APRON_Y),
    bpt(S05_FACE - 0.01, 0.005, APRON_Y),
]
verts = [tuple(pt) for pt in apron]
uvs = [(0, 0), (3.0 / 2.4, 0), (3.0 / 2.4, 1.51 / 2.4), (0, 1.51 / 2.4)]
L.mesh('b-apron', verts, [(0, 1, 2, 3)], 'paving', uvs)

# recessed drain channel across the walkable width, mid-apron
outlet = [
    bpt(-0.75, -0.55, OUTLET_Y), bpt(0.75, -0.55, OUTLET_Y),
    bpt(0.75, -0.35, OUTLET_Y), bpt(-0.75, -0.35, OUTLET_Y),
]
verts = [tuple(pt) for pt in outlet]
uvs = [(0, 0), (1.5 / 1.2, 0), (1.5 / 1.2, 0.20 / 1.2), (0, 0.20 / 1.2)]
L.mesh('b-drain-outlet', verts, [(0, 1, 2, 3)], 'stone', uvs)

# pier-base bricks bridging wall-end -> portal pier at the REAL corners
# (production bpt), grounded; sized so they never intrude the 1.5 m clear
# opening (lane-lx extent stays outside +-0.75) and reach into both the
# S05/S07 wall bodies and the pier zone
for sgn, nm in ((-1, 'w'), (+1, 'e')):
    c = bpt(sgn * 0.97, -0.20)
    L.box(f'b-pier-base-{nm}', (c[0], 0.30, c[2]), (0.40, 0.6, 0.16), 'brick', 0.006, True)
...
design = {
    ...
    'sampleId': 'lane-b-interfaces-v3',
    'basedOn': 'kit/build_lanes_interfaces_v2.py (mouth A verbatim) + measured B mouth lines (survey 2026-09-20): S05 face lx=-1.096, S07 face line lx(ls)=1.067-0.0536*(ls+0.26), holder 57.418,0.09,14.2485 yaw -0.4818',
    ...
    'measuredAnchors': {
        ...
        'bMouth': {
            's05FaceLx': S05_FACE, 's07FaceAtMouthLx': round(S07_FACE_AT(-0.2), 4),
            's07FaceTiltPerM': -0.0536,
            'bApronPolygonXz': [[round(v[0], 4), round(v[2], 4)] for v in apron],
            'apronY': APRON_Y, 'outletY': OUTLET_Y,
            'pierBaseCenters': {'w': [round(v, 4) for v in bpt(-0.97, -0.20)],
                                'e': [round(v, 4) for v in bpt(0.97, -0.20)]},
        },
    },
    'floorPolygonXz': [[round(x, 4), round(z, 4)] for (x, z) in FLOOR_PTS],
    'changesVsV6': [
        'mouth A geometry identical to v6 (a-floor/a-drain/a-threshold/east mouth wall/jambs code copied verbatim)',
        'B_RIGHT fixed to the production +Y rotation sign; quad() fed [x,z] pairs (no z-collapse)',
        'b-apron re-fitted to the measured S05/S07 mouth (was +-1.5 authored, collapsed to the origin)',
        'b-drain-outlet recessed channel at the walkable width, 18 mm under the apron',
        'b-pier-base w/e at the real corner positions (production bpt), grounded y0..0.6',
    ],
...
tris, bytes_ = L.finalize('lane-b-interfaces-v3', a.out, design)
print(f'INTERFACES_V3_READY tris={tris} bytes={bytes_}')
assert tris <= 1000, f'interfaces-v3 {tris} > 1000 budget'
