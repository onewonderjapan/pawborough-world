// Temple AXIS V2 passage tests — REAL Rapier, the REAL assembled dataset
// (world/temple-axis-v2/): ground trimesh from the VISIBLE ground meshes of
...
// front) and back to the street. Negatives: the six structured
...
const INSTANCE_OF = {
  'ground.glb': [[0, 0, 0], 0], 'court-open.glb': [[0, 0, 0], 0],
...
const groundMeshes = [];
for (const file of ['ground.glb', 'court-open.glb', 'dadian-court-v2.glb', 'court3.glb',
...
// --- 1. positive: gates + stage passage --------------------------------------
{
  ...
  const marks = [];
  let gi = 0;
  const wps = [[0, -24.5], [0, -28.0], [-4.0, -31.0]];
  let wi = 0;
...
// --- 4. negatives: the six structured DESIGN_SPEC.route.negatives ---------------
{
...
console.log(failures === 0 ? '\nAXIS_V2_PASSAGE PASS' : `\nAXIS_V2_PASSAGE FAIL (${failures})`);
