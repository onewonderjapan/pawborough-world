// Houdian (north hall, 城隍殿) contract tests — the kit build against the
// frozen DESIGN_SPEC.houdian numbers: validator-clean GLB, bounding box,
// column grid, closed-door width, BLANK plaque (zero novel textures),
// triangle + ornament budgets, monotone roof surface (ridge 8.0 flat, 硬山),
// ground naming, collision set.
//
// Run: node tests/houdian_contract.test.mjs   (exit 0 = contract holds)
...
const KIT = resolve(root, 'kit/out/houdian');
let failures = 0;
...
const bytes = await readFile(resolve(KIT, 'houdian.glb'));
...
// C1 validator + budget + manifest integrity
{
  ...
  check('C1: triangles <= 22,000', measure.targets['houdian.glb'].triangles <= 22000,
    `${measure.targets['houdian.glb'].triangles}`);
  check('C1: sha in measurements matches the bytes',
    measure.targets['houdian.glb'].sha256 === (await import('node:crypto')).createHash('sha256').update(bytes).digest('hex'));
...
{
  const b = BOUNDS;
  check('C2: min x ~ -7.0 (base extent)', Math.abs(b.min[0] + 7.0) < 0.02, `${b.min[0].toFixed(3)}`);
  check('C2: max x ~ +7.0', Math.abs(b.max[0] - 7.0) < 0.02, `${b.max[0].toFixed(3)}`);
  check('C2: rear extent ~ -9.0 (rear overhang 0.6)', Math.abs(b.min[2] + 9.0) < 0.05, `${b.min[2].toFixed(3)}`);
  check('C2: front extent ~ +2.0 (base/eave line)', Math.abs(b.max[2] - 2.0) < 0.05, `${b.max[2].toFixed(3)}`);
  check('C2: top ~ ridge + beam + kiss (8.0+0.36+~0.55)', b.max[1] > 8.5 && b.max[1] < 9.1, `${b.max[1].toFixed(3)}`);
...
{
  const cols = collision.colliders.filter((c) => c.name === 'houdian-column');
  check('C3: 4 gallery column colliders', cols.length === 4, `${cols.length}`);
  const xs = cols.map((c) => c.obb.center[0]).sort((a, b) => a - b);
  const want = [-6.3, -2.1, 2.1, 6.3];
  check('C3: column x at ±1.1/±3.3', want.every((v, i) => Math.abs(xs[i] - v) < 1e-6),
    xs.map((v) => v.toFixed(2)).join(','));
  check('C3: column z at 1.2', cols.every((c) => Math.abs(c.obb.center[2] - 1.2) < 1e-6));
  check('C3: column height 4.4', cols.every((c) => Math.abs(c.obb.size[1] - 4.4) < 1e-6));
}
...
// C4 closed door 1.6 wide (two 0.8 leaves), threshold stone
{
  ...
  check('C4: leaves fill 2.4 m', Math.abs(total - 2.4) < 1e-6, `${total}`);
  check('C4: leaf height 3.2', leaves.every((c) => Math.abs(c.obb.size[1] - 3.2) < 1e-6));
...
// C6 roof surface: monotone fall, >=4 x >=24 grid, flat ridge at 6.2
{
  ...
  for (const leg of ['front', 'rear']) {
    ...
    for (const row of roof[leg]) {
      if (row.t === 0) for (const y of row.y) if (Math.abs(y - 8.0) > 1e-6) ridgeFlat = false;
    }
  }
  check('C6: surface never rises toward the eave (both slopes, all columns)', monotone);
  check('C6: ridge stays flat at 8.0 (硬山, no hip falloff)', ridgeFlat);
}
...
// C7 ground naming + collision set (spec: 柱4 侧墙2 后墙1 闭门1 格扇面2 台基侧面3)
{
  ...
  check('C7: 4 columns', count('houdian-column') === 4);
