// Build the standalone dist for the lane-b-polish candidate — vite build to
// its OWN outDir (dist-lane-b-polish, --emptyOutDir false: never wipes prior
// deliverables), then carry the full fangbang-temple-v7 dependency closure so
// the page actually RUNS from the directory (not just compiled JS): dataset
// files + every site-root GLB/collision the v7 blocks and manifest reference
// (lane-a keeps its v6 asset bytes; lane-b/interfaces are the new files) +
// the optional skins/props datasets + building modules. A required-file gate
// fails the build on anything missing; a headless preview smoke (real page
// load + automatic route check, both display configurations) closes it.
//
// Run: node scripts/build_lane_b_polish_dist.mjs
...
const OUT = 'dist-lane-b-polish';
const DATASET = 'fangbang-temple-v7';
// measured base/all-on triangle counts for THIS candidate (the page's own
// accounting; the dist gate must reproduce them exactly)
...
await writeFile(resolve(root, OUT, 'lane-b-polish-dist-inventory.json'), JSON.stringify({
  generatedBy: 'scripts/build_lane_b_polish_dist.mjs',
  dataset: DATASET, files: inventory,
  totalBytes: Object.values(inventory).reduce((s, v) => s + v, 0),
}, null, 2) + '\n');
console.log(`LANE_B_POLISH_DIST_READY files=${Object.keys(inventory).length} bytes=${Object.values(inventory).reduce((s, v) => s + v, 0)}`);
...
const loadConfig = async (query) => {
  ...
  page.on('requestfailed', (r) => { if (!optional(r.url())) problems.push(`requestfailed ${new URL(r.url()).pathname}: ${r.failure()?.errorText}`); });
  page.on('response', (r) => { if (r.status() >= 400 && !optional(r.url())) problems.push(`http${r.status()} ${new URL(r.url()).pathname}`); });
  try {
...
try {
  ...
  ok = base.ready && base.route === true && base.tris === EXPECT_BASE
    && allOn.ready && allOn.route === true && allOn.tris === EXPECT_ALLON;
...
await browser.close();
prev.kill();
if (budgets) await writeFile(resolve(root, OUT, 'lane-b-polish-dist-run.json'), JSON.stringify({
  gate: 'headless Chrome + SwiftShader over vite preview of dist-lane-b-polish',
