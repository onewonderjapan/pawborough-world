// Build the standalone dist for BOTH pilot pages (temple.html 5291,
// temple-entry.html 5293) and carry the world datasets inside it — a dist
// that 404s (or SPA-fallbacks) the GLBs serves a dead page, which is exactly
// the state 5291 was delivered in before this fix. Runs vite build, then
// copies the datasets and verifies every required file exists (hard fail).
...
const DATASETS = ['world/temple-shanmen', 'world/temple-entry'];
...
// required-file gate: the dist either carries both pilots complete or fails
const REQUIRED = [
  'index.html', 'temple.html', 'temple-entry.html',
  ...DATASETS.flatMap((ds) => {
...
await writeFile(resolve(root, 'dist/dist-inventory.json'),
  JSON.stringify({ generatedBy: 'scripts/build_entry_dist.mjs', files: inventory }, null, 2) + '\n', 'utf8');
