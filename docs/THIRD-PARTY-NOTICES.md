...
GLB、Blend、贴图、照片、视频和渲染帧目前仅公开Git LFS指针与哈希清单，未上传实体。资产不因公开仓库而自动获得与源码相同的许可。后续发布应保留各素材的实际来源、作者、许可及适用署名条件。生成推理图标记design_inference，不当作历史照片或测绘依据。无法确定可再分发范围的素材保持未发布，不用猜测来源补许可证。
