// Temple AXIS V2 viewer — the expanded temple axis (temple-v2.html, ports
// 5298/5299), sibling of the dadian page and built from it: same production
// modules and the SAME shared viewer core (src/templeViewShared.js) — no
// second engine. Loads the assembled dataset world/temple-axis-v2/: the seven
// frozen axis assets, the dadian-court-v2 VARIANT, the stage on the yimen
// rear, peidian x2 and gallery x2 at yaw ±π/2, the hall, court3 and the
// houdian at (0,0,-74). Ten lead cameras, walk mode on the same physics
// chain, and an automated route cruise (honestly labeled automatic — the full
// walk to the houdian doors, six structured negatives, return, fall checks).
...
const BASE = './world/temple-axis-v2/';
const DATASET_TAG = 'temple-axis-v2';
const LABELS = {
...
function record() {
  return {
    dataset: 'temple-axis-v2', view: selected, mode, clay: clayOn,
...
async function runRouteCheck() {
  ...
  };
  {
    ...
    const wps1 = [[0, -24.5], [0, -28.0], [-4.0, -31.0]];
    let wi = 0;
    ...
    const okReturn = await follow(c, [[0, -70.0], [0, -66.0], [8.0, -65.0], [13.5, -60.0],
      ...
      [-8.5, -35.8], [-8.5, -31.5], [-4.0, -30.5], [0, -28.0], [0, -12], [0, 4.2]], 0.55, 300);
...
async function load() {
  ...
  const loaded = { temple, ground, lions, ornaments, yimen, courtOpen, dadianCourt,
    peidian: peidianW, gallery: galleryW, yimenStage: stage, dadian, court3, houdian };
  ...
  loadStats = {
    ...
    bytesNew: dadianCourt.bytes + peidianW.bytes + galleryW.bytes + stage.bytes
      + court3.bytes + houdian.bytes,
