...
// Release the GPU resources a loaded asset group owns (geometry, materials,
// textures) and detach it. Never touches physics — collider removal lives in
// discardAssets / the BlockManager, which know whether the world is alive.
...
export function createBlockViews(scene, session = null) {
  ...
  const api = {
    ...
    async makeAssets(def, opts = {}) {
      if (!session) throw new Error('blockViews: assets blocks require the loaded world session');
      const isCancelled = opts.isCancelled ?? (() => false);
      // injectable await boundaries (production defaults): node tests drive
      // THIS factory with gated/failing fetches; the browser always runs the
      // real fetch + GLTFLoader path
      const fetchImpl = opts.fetchImpl ?? ((u, init) => fetch(u, init));
      const parseImpl = opts.parseImpl ?? ((buf, url) => loader.parseAsync(buf, url));
      ...
      const world = session.physics.world;
      const colliders = [];
      const cancelled = () => Object.assign(new Error(`asset block ${def.id}: load cancelled mid-flight`), { cancelled: true });
      try {
        for (const asset of def.assets) {
          // every await boundary re-checks cancellation: an unload/dispose
          // during fetch or parse must not create further Rapier bodies
          ...
          if (isCancelled()) throw cancelled();
          const model = await parseImpl(bytes, url);
          if (isCancelled()) throw cancelled();
          ...
          group.add(holder);
          if (asset.collision) {
            const cres = await fetchImpl(new URL(asset.collision, location.href).href, { cache: 'no-cache' });
            if (isCancelled()) throw cancelled();
        ...
        // full self-rollback of everything created so far — a failed or
        // cancelled load leaves NO collider behind and disposes its decoded
        // GPU resources; after owner teardown (session.disposed) the freed
        // physics world is left alone and the colliders die with it
        if (session.disposed !== true) {
          for (const c of colliders) removeColliderWithBody(world, c.collider, c.body);
        }
    ...
    disposeAssets(parts) {
      ...
      disposeGroupResources(parts.group);
    },
