...
//
// Run: node tools/capture_temple_v3_views.mjs --base http://127.0.0.1:5306 --out artifacts/corridor/web
...
const out = resolve(root, arg('--out', 'artifacts/corridor/web'));
...
await page.goto(base + '/temple-v3.html', { waitUntil: 'domcontentloaded' });
await page.waitForFunction((g) => typeof window[g] === 'function', '__templeV3Record', { timeout: 300000 });
...
for (const id of viewIds) {
  ...
  const file = resolve(out, `v3-web--${id.replace(/:/g, '__')}.png`);
...
await writeFile(resolve(out, 'capture-report.json'), JSON.stringify({
  base, views: report, pass: fails === 0, generatedBy: 'tools/capture_temple_v3_views.mjs',
}, null, 2) + '\n');
await browser.close();
