// fangbang-temple bridge performance baseline (V09 口径 — measure, never
// claim). Headless Chrome with SwiftShader (software GL, NOT the target
// device): page load bytes/timing, first rendered frame, automatic route
// cruise frame-time percentiles (P50/P95), resource counts, and 3×
// enter/exit resource growth (leak check). Output: perf.json — a REGRESSION
// BASELINE ONLY, no FPS claim.
//
// Run (dev server on 5296 already up):
//   node tools/fangbang_perf.mjs --out artifacts/fangbang-temple/perf.json
...
const OUT = resolve(root, process.argv.includes('--out') ? process.argv[process.argv.indexOf('--out') + 1] : 'artifacts/fangbang-temple/perf.json');
