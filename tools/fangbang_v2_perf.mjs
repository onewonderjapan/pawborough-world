// fangbang-temple-v2 bridge performance baseline (expansion batch N8; V09
// 口径 — measure, never claim). Copy of tools/fangbang_perf.mjs with the
// ?ds=fangbang-temple-v2 query added (the delivered tool is read-only). 
...
// resource growth (leak check). Output: perf-v2.json — a REGRESSION BASELINE
// ONLY, no FPS claim.
//
// Run (dev server on 5300 already up; 5296 occupied per fallback #9):
//   BASE_URL=http://127.0.0.1:5300 node tools/fangbang_v2_perf.mjs \
//       --out artifacts/temple-expansion/perf-v2.json
...
const OUT = resolve(root, process.argv.includes('--out') ? process.argv[process.argv.indexOf('--out') + 1] : 'artifacts/temple-expansion/perf-v2.json');
