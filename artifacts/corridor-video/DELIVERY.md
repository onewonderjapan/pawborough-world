...
## GATE 附加项（原文，机主裁决；施工者不进入）

| # | 决策 | 选项 | 影响 |
| --- | --- | --- | --- |
| G11 | **旧化层**：首轮评审建议全世界补污渍/剥落/漆面旧化；机主 2026-09-13 决定『不做旧』 | 保持不做旧 / 推翻并另开批（需定旧化程度与年代口径） | 决定 v1.0 画面基调 |
| G12 | **树与道具层**（庙院 4 樟树、街道 38 件 1990 年代道具） | 保留 / revoke / 改数量或位置 | 整块可撤 |
| G13 | **狮子 v2 与方形漏窗 v2** | 采用 / 回 v1 | temple-axis-v3 变体 |
| G14 | **巡游视频** clipA/clipB 是否作为 v1.0 候选包附件 | 附 / 不附 / 换机位重渲 | 对外用途另议 |

附注（本批新产生、待机主追认）：axis-aerial 姿态调整（spec 姿态实测 0.22–0.26 达不到其自带 ≥0.40 验收线，调整后 0.435；对比图 kit/out/scene-v2/camera-fix__axis-aerial-{old,new}-cpu.png）；树位 fallback #2 平移 3 棵（manifest.placementShifts）；clipA 91% partial（9h 帽，fallback #11）。
